...
public class AdminController {
    ...
    @Autowired
    public AdminController(UserRepository ur, AdminRepository ar, ManagerRepository mr, TeacherRepository tr, StudentRepository str, ClassRepository cr, SubjectRepository sjr, UserService us) {
        this.ur = ur;
        this.ar = ar;
        this.mr = mr;
        this.tr = tr;
...
//                    Class c = currentClasses.get(i);
//                    currentClasses.set(i,cr.findClassById(c.getClassCode()));
//                }
                System.out.println("Class assignment info for teacher " + t);
                System.out.println("Current classes: " + currentClasses);
                System.out.println("Remaining classes: " + remainingClasses);
...
//            System.out.println("Erroneous admin caught: " + a);
    ...
    public String saveUserTeacher(@ModelAttribute("user") Teacher t, @RequestParam(value="oldRole", required=false) String oldRole, RedirectAttributes ra){
        ...
        t.setUserId((t.getUser().getUserId()));
        us.save(t.getUser(),"{bcrypt}");
        t.setSubject(sjr.findSubjectById(t.getSubject().getSubjectId()));
//        Re-populating the list with managed entities
            for(int i=0; i < currentClasses.size(); i++){
                Class c = currentClasses.get(i);
                currentClasses.set(i,cr.findClassById(c.getClassCode()));
            }
...
//            System.out.println("Excel file path: " + excelPath);
            ...
            switch(role) {
                case "admin":
                    ...
                    excelHelper = new ExcelHelper(excelPath, fieldNames);
                    data = excelHelper.readData();
...
//Classes
    ...
    public String loadImportClasses(){
        return "admin/file-import-class";
    }
...
//            System.out.println("Uploaded file name: " + fileName);
            ...
            ExcelHelper excelHelper = new ExcelHelper(excelPath,fieldNames);
            List<Map<String, String>> data = excelHelper.readData();
            System.out.println("Data from Excel file: " + data);
...
//        System.out.println();
    ...
    public String addClassToTeacher(@RequestParam("userId") int utid, @RequestParam("classCode") int accode, RedirectAttributes ra){
        Teacher t = tr.findTeacherById(utid);
        if(t==null) throw new ApiRequestException("User not found.");
        Class c = cr.findClassById(accode);
        if(c==null) throw new ApiRequestException("Class not found.");
