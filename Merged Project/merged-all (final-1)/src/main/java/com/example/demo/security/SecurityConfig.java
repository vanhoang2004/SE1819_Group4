	...
	public AuthenticationProvider authenticationProvider() {
		...
		return new AuthenticationProvider() {
			...
			public Authentication authenticate(Authentication authentication) throws AuthenticationException {
				...
				if(userDetailsPassword.startsWith("{bcrypt}")) userDetailsPassword = userDetailsPassword.substring(8);

				if (!noOpEncoder().matches(password, userDetailsPassword) && !bcryptEncoder().matches(password, userDetailsPassword)) {
